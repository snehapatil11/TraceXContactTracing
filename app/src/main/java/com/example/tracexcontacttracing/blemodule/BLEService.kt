...
class BLEService: Service() {
    ...
    private val mGattCallback = object : BluetoothGattCallback() {
        override fun onConnectionStateChange(gatt: BluetoothGatt, status: Int, newState: Int) {//Change in connection state
            ...
            if (newState == BluetoothProfile.STATE_CONNECTED) {//See if we are connected
                ...
                broadcastUpdate(BConstants.ACTION_GATT_CONNECTED)//Go broadcast an intent to say we are connected
                ...
                broadcastUpdate(BConstants.ACTION_GATT_DISCONNECTED)//Go broadcast an intent to say we are disconnected
            }
        }
        ...
        override fun onServicesDiscovered(gatt: BluetoothGatt, status: Int) {              //BLE service discovery complete
            if (status == BluetoothGatt.GATT_SUCCESS) {                                 //See if the service discovery was successful
                ...
                broadcastUpdate(BConstants.ACTION_GATT_SERVICES_DISCOVERED)                       //Go broadcast an intent to say we have discovered services
        ...
        override fun onCharacteristicRead(gatt: BluetoothGatt, characteristic: BluetoothGattCharacteristic, status: Int) { //A request to Read has completed
            ...
            ConnectionManagerBLE.writeEmergencyGatt(clearValue)


            if (status == BluetoothGatt.GATT_SUCCESS) {
                //See if the read was successful
                Log.i(TAG, "**ACTION_DATA_READ**$characteristic")
                broadcastUpdate(BConstants.ACTION_DATA_AVAILABLE, characteristic)                 //Go broadcast an intent with the characteristic data
            } else {
        ...
        override fun onCharacteristicWrite(gatt: BluetoothGatt, characteristic: BluetoothGattCharacteristic, status: Int) { //A request to Write has completed
            if (status == BluetoothGatt.GATT_SUCCESS) {                                 //See if the write was successful
                ...
                broadcastUpdate(BConstants.ACTION_DATA_WRITTEN, characteristic)                   //Go broadcast an intent to say we have have written data
            }
        }
        ...
        override fun onCharacteristicChanged(gatt: BluetoothGatt, characteristic: BluetoothGattCharacteristic?) {
            ...
            if (characteristic != null) {
                broadcastUpdate(BConstants.ACTION_DATA_AVAILABLE, characteristic)
    ...
    private fun broadcastUpdate(action: String, characteristic: BluetoothGattCharacteristic) {
        ...
        if (value != null) {
            ...
            intent.putExtra(BConstants.EXTRA_DATA, dataValueNew)
            intent.putExtra(BConstants.EXTRA_UUID, characteristic.uuid)
    ...
    private fun processIntent(intent: Intent?, isNeedToSend: Boolean) {
        if (isNeedToSend) {
            ...
            intent!!.putExtra(BConstants.EXTRA_DATA, value)
            sendBroadcast(intent)
        }
