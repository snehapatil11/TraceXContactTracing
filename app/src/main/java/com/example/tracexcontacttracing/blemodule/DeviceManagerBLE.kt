...
object DeviceManagerBLE {
    private val TAG = "BLEDeviceManager"
    private var scanCallback: ScanCallback? = null
    private var mDeviceObject: DeviceData? = null
        ...
        scanCallback = object : ScanCallback() {
            override fun onScanResult(callbackType: Int, result: ScanResult?) {
                ...
                if (null != mOnDeviceScanListener && result != null &&
                     ...
                     */
                    println(data.mDeviceName)
                    println(data.mDeviceAddress)
    ...
    private fun scan() {
        if (isLollyPopOrAbove()) {// Start Scanning For Lollipop devices
            mBluetoothAdapter?.bluetoothLeScanner?.startScan(scanCallback) // Start BLE device Scanning in a separate thread
        } else {
    ...
    private fun stopScan(data: DeviceData?) {
        try {
